...
def evaluate_forward(data, model, name):
    ...
    for batch in data.batch_iter(name, batch_size, False):
        gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen, batch_charrecover, ins_label, batch_label, mask, scope = batch
        prob = model(gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen, batch_charrecover,
                     ins_label, scope)
...
def load_eval(data, model_dir, name):
    ...
    f = open('log.txt', 'a')
    f.write("Pre: %.4f; Rec: %.4f; f1: %.4f; acc: %.4f" % (precision, recall, f1, auc))
    print("Test: time: %.2fs; Pre: %.4f; Rec: %.4f; f1: %.4f; acc: %.4f" % (time_cost, precision, recall, f1, auc))
...
def train(data, save_model_dir):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch in data.batch_iter('train', batch_size):
            gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen, batch_charrecover, ins_label, batch_label, mask, scope = batch
            prob = model(gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen,
                         batch_charrecover, ins_label, scope)  # batch_size*num_classes
            batch_loss = loss_fn(prob, batch_label)
            # print(batch_loss.cpu().item())
            sys.stdout.write('Loss: {}%'.format(batch_loss.cpu().item()) + '\r')
            sys.stdout.flush()
        ...
        f = open('log.txt', 'a')
        f.write("Pre: %.4f; Rec: %.4f; f1: %.4f; acc: %.4f" % (precision, recall, f1, auc))
        print("Dev: time: %.2fs;Pre: %.4f; Rec: %.4f; f1: %.4f; auc: %.4f" % (dev_cost, precision, recall, f1, auc))
        if auc > best_auc:
            print('Current acc ', auc, 'exceed previous best acc:', best_auc)
            if f1 <= best_f1:
        ...
        print("Test: time: %.2fs;Pre: %.4f; Rec: %.4f; f1: %.4f; acc: %.4f" % (test_cost, precision, recall, f1, auc))
        print("============================================================")
